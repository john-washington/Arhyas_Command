...
found=$(find "${data_dir}" -type f -name "${host}_trace_result.txt" -print0)

#if [[ -s  "${data_dir}/${host}_trace_result.txt" ]]; then

if [[ $(wc -c < "$found" ) -eq 0 ]]; then

	command_str="traceroute  "${host}" |  bash ./tracelist.sh > '${data_dir}/${host}_trace_result.txt'"
	echo "${command_str}"
    eval "${command_str}"
...
found1=$(find "${data_dir}" -type f -name "${host}_trace_result.txt_geo_data.csv" -print0 ) 

#if [ -f  "${data_dir}/${host}_trace_result.txt_geo_data.csv" ] && [ $(wc -c < "${data_dir}/${host}_trace_result.txt_geo_data.csv") -gt 2]; then
if [[ $(wc -c < "$found1" ) -eq 0  ||  $(wc -c < "${data_dir}/${host}_trace_result.txt_geo_data.csv" ) -le 2 ]]; then
	bash ./ip-api.sh -b "${data_dir}/${host}_trace_result.txt" | tee -a "${log_dir}/arhyas_command.log"
else
	echo "${data_dir}/${host}_trace_result.txt_geo_data.csv already exist and not empty" | tee -a "${log_dir}/arhyas_command.log"
fi
...
if [ $status -eq 124 ]; then
	...
	cat "${data_dir}/${host}_trace_result.txt" en  | xargs -n 2 bash  ./arhyas_msg.sh 
elif [ $status -ne 0 ]; then
	echo "command failed with status: $status";
	cat "${data_dir}/${host}_trace_result.txt" en  | xargs -n 2 bash ./arhyas_msg.sh 
else
	bash ./arhyas_msg.sh "$host" en
fi
