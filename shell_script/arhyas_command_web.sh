...
radius=$3
lang_code=$4
#echo $lat $lon $radius $lang_code > input.txt
...
#echo "column count" "${column_count}"
  ...
  #echo "calling ip-api.sh with: lat:${lat} lon:${lon} radius:${radius} lang_code: $[lang_code}"

  #cm="${script_dir}/ip-api.sh.x -w '${lat}, ${lon}, ${radius}, ${lang_code}' | tee -a ${log_dir}/Arhyas_Command.log"

  ${script_dir}/ip-api.sh.x -w "${lat},${lon},${radius},${lang_code}" | tee -a ${log_dir}/Arhyas_Command.log
