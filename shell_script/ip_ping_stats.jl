...
function generate_ping_stats_graph(filename)
  try
    ...
    packet_traves_time_sequence = [PING_DICT[1, 4]]
    ...
    for i in 1:size(PING_DICT, 1)
      ...
      push!(packet_traves_time_sequence, PING_DICT[i, 4])
    end
    ...
    plot!(time_sequence, packet_traves_time_sequence, label="packet traverse time(ms)", color="red")
